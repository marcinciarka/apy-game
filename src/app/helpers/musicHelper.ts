...
const bassPattern = [
  36, 0, 36, 0, 39, 0, 36, 0, 41, 0, 36, 0, 43, 41, 39, 36, 36, 0, 36, 0, 39, 0,
  36, 0, 44, 0, 41, 0, 45, 44, 41, 39,
];
...
function scheduleNote(noteTime: number) {
  ...
  if (bassNote > 0) {
    ...
    noteGain.gain.exponentialRampToValueAtTime(
      0.01,
      noteTime + secondsPer16thNote * 0.9
    ); // Quick decay
  ...
  if (current16thNote % 4 === 0 && arpNote > 0) {
    ...
    for (let i = 0; i < bufferSize; i++) {
      data[i] = (Math.random() * 2 - 1) * Math.exp(-i / (bufferSize * 0.7)); // Exponential decay
    }
  ...
  if (current16thNote % 128 === 112) {
    ...
    for (let i = 0; i < 8; i++) {
      const fillTime = noteTime + i * (secondsPer16thNote / 8);
      const bufferSize = audioContext.sampleRate * 0.04; // ~40ms
      const buffer = audioContext.createBuffer(
        1,
        bufferSize,
  ...
  if (current16thNote % 16 === 0) {
    ...
    chordNotes.forEach((note, i) => {
      ...
      pan.pan.setValueAtTime(-0.5 + i * 0.5, noteTime); // spread voices
      ...
      chordGain.gain.linearRampToValueAtTime(
        0.005,
        noteTime + secondsPer16thNote * 16 - 0.1
      );
      // Routing
      osc1.connect(pan);
      osc2.connect(pan);
      ...
      osc2.start(noteTime);
      osc3.start(noteTime);
      osc1.stop(noteTime + secondsPer16thNote * 16);
      osc2.stop(noteTime + secondsPer16thNote * 16);
      osc3.stop(noteTime + secondsPer16thNote * 16);
    });
  }
}
